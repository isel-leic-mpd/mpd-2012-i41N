...
public class EmployeesDataMapper extends TemplateDataMapper<Integer, Employee>{
	final static String sqlLoadAll = "SELECT EmployeeId, Title, FirstName, LastName, BirthDate, ReportsTo FROM Employees";
	...
	final static String sqlLoadByReportsTo = sqlLoadAll + " WHERE reportsTo = ?";
			
	final JdbcConverter<Iterable<Employee>> conv = new JdbcConverter<Iterable<Employee>>(){
		@Override
		public Iterable<Employee> convert(final ResultSet rs) throws SQLException{
			LinkedList<Employee> res = new LinkedList<Employee>();
			while(rs.next()){
				final int id = rs.getInt(1);
				Employee e = getEntity(id);
				if(e == null){
					Iterable<Employee> employees = new Iterable<Employee>() {	
						@Override
						public Iterator<Employee> iterator() {
							try {
								return loadByReportsTo(id).iterator();
							} catch (SQLException e) {
								throw new RuntimeException();
							}
						}
					};
					final int managerId = rs.getInt(6);
					/*
					Employee reportsTo = new Employee(){
						Employee cache = null;
						private Employee initCache(){
							if(cache == null)
								try {
									cache = loadById(managerId);
								} catch (SQLException e) {
						...
						public String getTitle(){
							return initCache().getTitle();
						}
						public String getFirstName(){
							return initCache().getFirstName();
						}
					};
					*/
					ValueHolder<Employee> reportsTo = new ValueHolder<Employee>() {
						@Override
						public Employee get() {
							try {
								return loadById(managerId);
							} catch (SQLException e) {
								throw new RuntimeException(e);
							}
					...
					e = new Employee(id,rs.getString(2),rs.getString(3),rs.getString(4),rs.getDate(5), employees, reportsTo);
					addEntity(id, e);
				}
				res.add(e);
			}
	...
	private Iterable<Employee> loadByReportsTo(int id) throws SQLException{
		JdbcCmdQuery<Iterable<Employee>> cmd = new JdbcCmdQueryTemplate<Iterable<Employee>>(
				sqlLoadByReportsTo,
				conv,
				JdbcBinder.BindInt);
		return db.executeQuery(cmd, id);
	}
